<select name="<?php _e( $field['name'] );?>" class="<?php _e( $field['class'] );?>">
  ...
  <option <?php if( isset( $field['selected'] ) && $option['slug'] == $field['selected'] ){ _e("selected='selected'");}?> value="<?php _e( $option['slug'] )?>" data-state="<?php _e( $option['parent'] )?>"><?php _e( $option['title'] );?></option>
  <?php endforeach;?>
</select>
